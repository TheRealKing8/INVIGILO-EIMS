"""Reconcile the RBAC matrix after splitting ``exam.session.crud`` into
``exam.session.create`` and ``exam.session.view``.
...
permissions when they upgrade. Same idempotent reconcile pattern as
``0005_seed_academic_hierarchy_perms`` — re-apply all roles,
permissions, and role-permission edges from ``apps.accounts.seed``.
"""
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("accounts", "0005_seed_academic_hierarchy_perms"),
    ]
